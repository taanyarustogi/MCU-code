...
void setup_PLL(plldev_t pll, byte mult, uint32_t num, uint32_t denom)
{
	...
	write(SI5351_PLLA_ADDR + 0, (P3 & 0x0000FF00) >> 8);
	write(SI5351_PLLA_ADDR + 1, (P3 & 0x000000FF));
	write(SI5351_PLLA_ADDR + 2, (P1 & 0x00030000) >> 16);
	write(SI5351_PLLA_ADDR + 3, (P1 & 0x0000FF00) >> 8);
	write(SI5351_PLLA_ADDR + 4, (P1 & 0x000000FF));
	write(SI5351_PLLA_ADDR + 5, ((P3 & 0x000F0000) >> 12) | ((P2 & 0x000F0000) >> 16));
	write(SI5351_PLLA_ADDR + 6, (P2 & 0x0000FF00) >> 8);
	write(SI5351_PLLA_ADDR + 7, (P2 & 0x000000FF));   
}
...
void set_LO_freq(uint32_t freq)
{
	...
	word PHASE_OFFSET =  round(fvco/freq);
